...
GhSettings settings; //Game_Mode Difficulty and User_color
GameHandler * gh = NULL; //game and settings
...
static void consoleGameHandleCmdUserColor(Command parsedCmd) {
	// Multiplayers' game will not have choice for color
	if (settings.gameMode != GameModeSinglePlayer) consoleGameHandleCmdInvalid();

	else if (parsedCmd.validArg && (parsedCmd.arg1 == BLACK || parsedCmd.arg1 == WHITE)) {
...
static void consoleGameHandleCmdPrintCommands(char *pathtofile){
...
/*
True if the game has been loaded successfully.
*/
...
static CgMsg consoleGameRunSettingsState(bool useDefaultSettings) {
	...
	printf("-----------------------------------------------------------\n");
	printf("|Commands:\t\t\t\t\t\t  |\n|game_mode/difficulty/user_color/commands\t\t  |\n");
	printf("|print_settings/default                                   |\n");
	printf("|quit/start/reset/undo/move                               |\n");
	printf("-----------------------------------------------------------\n");
	printf("Specify game settings or type 'start' to begin a game with the current settings:\n");
	do {
		// get command

		if (fgets(cmd, MAX_LINE_LENGTH + 1, stdin) == NULL) { // EOF reached
		...
		case CMD_TYPE_PRINT_SETTINGS:
			...
			break;
		case CMD_TYPE_COMMAND_GUIDE:
			consoleGameHandleCmdPrintCommands("./Guide");
			break;
		case CMD_TYPE_QUIT:
			consoleGameHandleCmdQuit();
			return CgMsgQuit;
...
static BoardSquare consoleGameConvertArgsToBoardSquare(int row, char col) {
	return (BoardSquare) { row - 1, col - 'A' };  // using ASCII to find board cordinations
}
...
static void consoleGameHandleCmdUndoMove() {
	...
	// check if no history
	if (piecesArrayIsEmpty(gh->game->history)) {
	...
	for (int i = 0; i < 2; i++) {
		...
		if (piecesArrayIsEmpty(gh->game->history)) break;

		// save history element and undo move
		histElem = piecesArrayGetLast(gh->game->history);
...
static void consoleGameHandleComputerMove() {
	...
	histElemComputerMove = piecesArrayGetLast(gh->game->history);
...
void consoleGameRun() {
	...
	printf(" Chess by Sabuhi Huseynov\n");
