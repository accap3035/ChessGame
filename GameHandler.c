...
void gameHandlerGameElementAddedToHistory(GameHandler * gh) {
	if (piecesArraySize(gh->game->history) > GH_GAME_HISTORY_SIZE) {
		piecesArrayRemoveFirst(gh->game->history);
	}
}

void gameHandlerComputerTurn(GameHandler * gh) {
	Move suggestMove = minimaxSuggestMove(gh->game, gh->settings.difficultyLevel); //level is for depth of alpha beta pruningn
